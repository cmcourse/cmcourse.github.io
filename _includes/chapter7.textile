...
p=(#figure7-8). !{width:42%}img/7/circuitsF7.8.jpg(Figure 7.8)!:img/7/circuitsF7.8.jpg
...
p=(#figure7-9). !{width:42%}img/7/circuitsF7.9.jpg(Figure 7.9)!:img/7/circuitsF7.9.jpg
_Figure 7.9  Practical Model with Source Resistance, $R_S$, and Load Resistance, $R_L$_

Here the load is specifically represented as an equivalent resistance, $R_L$.  The source current iS equals the load current, $i_L$, because there is a single loop circuit.

Notice with the finite source resistance, $R_S$, when the load current increases ($R_L$ smaller), then the voltage at the load, $V_L$, decreases due to the voltage drop across $R_S$.  This reduction in load voltage, $V_L$, is expressed by Eq’n. 11, where by ohm’s law:

p=. $V_L = V_S - i_L R_S$

Here we can see when iL increases. $V_L$ must decrease.  So $V_L$ even becomes zero when $i_L$ is $V_S$/$i_S$.  But power into the load is the $V_L$ • $i_L$ product, and thus becomes zero at this current.
...
p=(#figure7-10). !{width:42%}img/7/circuitsF7.10.jpg(Figure 7.10)!:img/7/circuitsF7.10.jpg
