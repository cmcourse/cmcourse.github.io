...
<h4>
p=. $P_{total} = \sum_{ \ \mbox(all-elements)} (v_k) (i_k) = 0$ <h\4>

It is remarkable that this development used only (1) the basic element voltage (the difference of the corresponding node voltages) and (2) Kirchoff's Current Law at the circuit nodes.  This 'sum-of-power = 0' constraint applies independent of element properties and also applies at any moment of time. This development method has been applied in general by Tellegen and the general relation is called Tellegen's Theorem.
