...
<h4>
p=. $\sum_{j=1 \ \mbox(all - branches)}^m i_j = 0$ </h4> ($i_j$ = current _leaving_ on branch '$j$')

p=(#eq3-1). $\sum_{j=1 \ \mbox(all - branches)}^m i_j = 0$  Eq'n 3.1
($i_j$ = current _leaving_ on branch '$j$') </h4>         
...
<h4>
p=(#eq3-3). $\sum_{j=1 \ \mbox(closed - loop)}^m v_j = 0$ Eq'n 3-3
($v_j$ = _branch voltage-drop_ on element '$j$' in loop) </h4>         

where, the element voltage 'drops', $v_j$, are for each of all the '_m_' elements in the closed loop.  This can be illustrated by a 4-element loop example, as depicted in "Figure 3.4":#figure3-4,
