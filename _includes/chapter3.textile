...
<h4>
p=. $\sum_{j=1 \ \mbox(all - branches)}^m i_j = 0$ </h4>

p=. ($i_j$ = current _leaving_ on branch '$j$')

p=(#eq3-1). $\sum_{j=1 \ \mbox(all - branches)}^m i_j = 0$  Eq'n 3.1
($i_j$ = current _leaving_ on branch '$j$')        
...
<h4>
p=(#eq3-3). $\sum_{j=1 \ \mbox(closed - loop)}^m v_j = 0$
($v_j$ = _branch voltage-drop_ on element '$j$' in loop)  Eq'n 3-3</h4>         
