...
p=. ($i_j$ = current _leaving_ on branch '$j$')  Eq'n 3.1 
...
<h4>
p=(#eq3-3). $\sum_{j=1 \ \mbox(closed - loop)}^m v_j = 0$ </h4>

p=. ($v_j$ = _branch voltage-drop_ on element '$j$' in loop)  Eq'n 3-3        


<h4>
p=. $$\sum_{j=1 \mbox(closed - loop)}^{m} v_j = 0$$ </h4>
...
<h4>
p=(#eq3-6). $P_{total} = \sum_{ \ \mbox(all-branchs)} (v_k) (i_k) = 0$ </h4>
...
h4(#chapter3-contradiction). 3.4 - Constraint Conflicts

One might try to consider a circuit that is inherently '_impossible_' from a circuits point-of-view due to conflicting constraints.  For example consider a loop circuit composed of 3 voltage sources each of 5 volts, as connected in "Figure 3.6":#figure3-6.
