...
The reason for the polarity convention, where positive current direction is *always* pointed into the defined positive voltage terminal on the element is to have a consistent definition of power.  This is because power is the *product* of _current times voltage_.  So with the polarity convention we can ALWAYS state that the *power dissipated* by an element is just the product $(i_X * v_X)$, or $(v_X * i_X)$, "Eq'n. 2.3":#eq2-3.
