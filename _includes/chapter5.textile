...
h4(#chapter5-voltage-divider). 5.2 - Voltage Divider: Example Series Connection
...
h4(#chapter5-parallel-connection). 5.3 - Parallel Connection: Single Node-Pair Restriction
...
h4(#chapter5-current-divider). 5.4 - Current Divider: Example Parallel Connection
...
<h4>
p=. $i_{R1} = (\frac{R_2}{R_1+R_2}) i_S$ </h4>
and

<h4>
p=. $i_{R2} = (\frac{R_1}{R_1+R_2}) i_S$ </h4>
...
_TAKE CARE:_ The current in $R_1$ depends on the ratio of ${R_2}/(R_1+R_2)$, while the current in $R_2$ depends upon the ratio ${R_1}/(R_1+R_2)$.
