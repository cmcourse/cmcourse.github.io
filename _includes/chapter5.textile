...
The source voltage terms in numerator and denominator cancel, and this result reveals that the total current , $i_S$, splits_ into two currents, $i_{R1}$ and $i_{R2}$, according to the resistor values $R_1$ and R_2$ as follows:
...
We see that each resistor current is a fixed fraction of the total current, and thus this circuit is often termd a *_current divider_*.

_TAKE CARE:_ The current in $R_1$ depends on the ratio of ${R_2}/(R_1+R_2)$, while the current in $R_2$ depneds upon the ratio ${R_1}/(R_1+R_2)$.
