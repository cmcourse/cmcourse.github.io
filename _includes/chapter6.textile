...
Recall our second special topology the _single node-pair_ circuit structure, such as from Section 5, Figure 5.7a.  Here all elements of the circuit are connected so they only connect between 2 nodes.  Because the circuit has a node-pair, and because of our KVL voltage law, there can be only one single voltage across all elements in this circuit.  Figure 6.6 depicts this arrangement for 3 resistors and a fixed voltage source.
