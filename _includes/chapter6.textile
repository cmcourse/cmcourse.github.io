...
**  Note it is *WRONG* to think that $V_a$ is determined as a voltage divider with just the two resistors of 3 ohm and 6 ohms !  They are NOT in series !!
...
but now notice there are two 6 ohm resistors both connected at node Va to ground, and so are in parallel.  And that means they can be reduced to an equivalent $R_{eq2}$ = 3 ohms, via:
